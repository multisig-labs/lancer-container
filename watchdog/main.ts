...
const onChange = async (_: any) => {
  ...
  const uniqueSubnets = Array.from(
    new Set(subnets.map((subnet) => subnet.subnet_id)),
  );
  const config = {
  ...
  try {
    const existingConfig: Config = JSON.parse(
      await Deno.readTextFile("/avalanche/configs/node.json"),
    );
  ...
  const container = containers.find((container) =>
    container?.Names?.find((name) => name.includes("avalanche"))
  );
  if (container) {
